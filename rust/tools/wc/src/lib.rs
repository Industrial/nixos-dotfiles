...
//!
//! The CLI entrypoint composes a lazy [`Effect`] graph and runs it with
//! [`id_effect::run_blocking`], matching the other dotfiles coreutils tools.
...
/// Package version string (`wc --version`).
...
pub struct Settings {
    ...
    pub debug: bool,
    pub files0_from: Option<OsString>,
    pub total_when: TotalWhen,
}
...
/// Early parse outcomes that bypass the main [`Effect`] (help/version).
#[derive(Debug, Clone)]
pub enum ParsedCli {
    ...
    Version,
    /// Normal run: word-count each input.
    Run {
        /// Resolved flags and default columns.
        settings: Settings,
        /// Files and stdin markers (`Input::Stdin` / `Path`).
        inputs: Vec<Input>,
    },
}
...
/// Synchronous word-count pipeline (used inside [`wc_effect`]).
pub fn run_wc_sync(settings: &Settings, inputs: &[Input]) -> i32 {
    if settings.debug {
    ...
    let stdin_implicit_only = matches!(inputs, [Input::Stdin(StdinKind::Implicit)]);
    ...
    let number_width = field_width(inputs, &counts, &sum_counts, settings, stdin_implicit_only);
...
/// Lazy effect graph for `wc`: synchronous counting and formatting, `()` environment.
pub fn wc_effect(settings: Settings, inputs: Vec<Input>) -> Effect<i32, String, ()> {
    Effect::new(move |_| {
        let settings = settings;
        let inputs = inputs;
        Ok(run_wc_sync(&settings, &inputs))
    })
}
