# Agent setup for `dotfiles`

This is a deepsec scanning workspace. Project `dotfiles` was just registered
(target: `..`). Setup is incomplete — `data/dotfiles/INFO.md`
...
2. **Fill in `data/dotfiles/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id dotfiles
pnpm deepsec process --project-id dotfiles
```
