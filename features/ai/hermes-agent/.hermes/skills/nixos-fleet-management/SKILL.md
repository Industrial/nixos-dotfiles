...
### Version mismatches in fleet packages

When deploying a fleet, you may encounter build failures due to version mismatches, e.g., `alertmanager-elm-ui` requiring Elm 0.19.1 while the system provides Elm 0.19.2. Fix by creating an overlay that pins the required version for the specific package:

```nix
# overlay.nix
self: super: {
  alertmanager-elm-ui = super.alertmanager-elm-ui.override {
    elm = super.elm_0_19_1;
  };
}
```

Then apply the overlay in your flake.nix:

```nix
outputs = inputs @ {self, ...}: let
  # ...
  nixpkgs = inputs.nixpkgs.overlay {
    final: prev: {
      alertmanager-elm-ui = prev.alertmanager-elm-ui.override {
        elm = prev.elm_0_19_1;
      };
    };
  };
...
This ensures the derivation uses the correct Elm version without affecting other packages.
