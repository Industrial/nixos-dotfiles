...
name: new-skill
description: >
  Distill conversation knowledge into reusable Hermes Agent skills.
  Mine the current conversation (and any files or URLs it references) for durable agent knowledge
  that is not yet covered by this repo's skills. Research each gap thoroughly, then extend an existing
  skill or author a new grouped skill under ~/.hermes/skills/<group>/<skill-name>/.
  Planning and gap analysis are the primary deliverable until the user approves the skill plan.
tags: [skill-creation, knowledge-transfer, documentation, workflow]
---

# New Skill — distill conversation knowledge into ~/.hermes/skills/<group>/<skill-name>/

Mine the **current conversation** (and any files or URLs it references) for durable agent knowledge that is **not yet covered** by this repo's skills. Research each gap thoroughly, then **extend an existing skill** or **author a new grouped skill** under `~/.hermes/skills/<group>/<skill-name>/`.
...
Review `~/.hermes/skills/` and load these before starting:

| Skill | Role in this command |
|-------|----------------------|
| `~/.hermes/skills/create-skill/SKILL.md` | SKILL.md structure, frontmatter, anti-patterns, verification checklist |
| `~/.hermes/skills/concise-planning/SKILL.md` | Atomic steps, acceptance criteria per deliverable |
| `~/.hermes/skills/continuous-improvement/SKILL.md` | When to fold lessons into existing docs vs new skills |

Do **not** create skills in `~/.hermes/skills-cursor/` — that directory is Hermes-internal.
...
```bash
# List all skills (group = parent directory under .hermes/skills/)
...
- Keep repo-wide cross-cutting skills at `~/.hermes/skills/<skill-name>/` only when no group fits (e.g. `effect.ts-fundamentals`, `git-pushing`)
...
|---|-------|--------|--------|-----------|
| 1 | … | extend / new / new group | `~/.hermes/skills/...` | … |
...
```
~/.hermes/skills/<group>/<skill-name>/
...
## See also
- `~/.hermes/skills/<group>/<other>/SKILL.md`
- Official: [link]
```
...
- [ ] `name` matches directory and is unique under `~/.hermes/skills/`
- [ ] Group folder name is lowercase, domain-clear
- [ ] Follow class-level skill organization: prefer rich SKILL.md with `references/` directory over flat list of narrow skills
...
### Class-level library principles (added from session learnings)
- [ ] Prefer class-level skills over narrow one-session skills
- [ ] Update skills actively each session - a pass that does nothing is a missed opportunity
- [ ] Embed user preferences for style/tone/format directly in skill (not just memory)
- [ ] Encode workflow corrections as pitfalls or explicit steps
- [ ] Capture non-trivial techniques, fixes, workarounds that future sessions would benefit from
...
### Created
- `~/.hermes/skills/<group>/<name>/` — [one-line purpose]

### Extended
- `~/.hermes/skills/.../SKILL.md` — [what was added]
...
| Write skills in `~/.hermes/skills-cursor/` | Project skills in `~/.hermes/skills/` only |
...
1. Inventory `~/.hermes/skills/**/SKILL.md`
