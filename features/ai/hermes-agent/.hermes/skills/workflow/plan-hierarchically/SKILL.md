...
description: >-
  Produce a Maestro-native hierarchical implementation plan before writing code. Use for /plan-hierarchically, /id PLAN mode, or any non-trivial feature plan.
---
...
**Spec `work_type` enum (hard-fails `maestro spec validate` if wrong):**
must be one of `new-spec | spec-slice | change-request | initiative | maintenance | harness-improvement`.
Rule of thumb: if you're adding tests, behavior, or fixing bugs, it's **`change-request`**. Reserve `initiative` for new top-level product areas, `new-spec` for brand-new spec drafts that haven't been gated, and `harness-improvement` for Maestro/ID tooling itself.
...
**Two plan files, two purposes.** The narrative `.plan.md` is for humans; the `.plan.yaml` is for `maestro plan check`.

1. `.cursor/plans/<slug>.plan.md` — Markdown for humans. Frontmatter holds
   the Maestro pointers + todos (it is Markdown-with-frontmatter, not
   YAML-in-markdown). The body is the long-form narrative (decision log,
   waves, leaf detail).
2. `.cursor/plans/<slug>.plan.yaml` — **Pure YAML** consumed by
   `maestro plan check --task <tsk-id> --plan-file …`. Required shape:

   ```yaml
   intendedFiles: [<path>, ...]                 # paths the plan touches
   ...
   Reference the YAML from the frontmatter of the `.plan.md` so the linkage
   is explicit.

**Why both.** `maestro plan check` parses the file as a single YAML document
(top-to-bottom). If you point it at a Markdown body that contains a second
`---` divider, it reports "Source contains multiple documents" and refuses;
`check-coverage-100-sweep.plan.md` did exactly this. The fix is the separate
`.plan.yaml`, not stripping the divider.

### Plan-check gate (CLI — no MCP tool)

After the plan files exist AND the task is claimed so a contract exists:

```bash
devenv shell -- maestro plan check --task <tsk-id> --plan-file .cursor/plans/<slug>.plan.yaml
```

**`maestro plan check` requires the task to be claimed.** A draft task has
no contract, so the gate returns `CONTRACT_NOT_FOUND` and the check is
not enforceable. Two options:

- Run the check after the user approves and you claim (preferred — the
  gate is then meaningful against the live contract).
- If you want a pre-approval sanity pass, validate the YAML structurally
  (`yq .cursor/plans/<slug>.plan.yaml > /dev/null`) and confirm the
  spec validates (`maestro spec validate .maestro/specs/<slug>.md`); record
  the structural pass via `maestro_evidence_record { kind: "manual-note" }`.
...
- Pointing `maestro plan check` at a `.plan.md` (Markdown body) — the YAML parser chokes on the second `---` divider; always use the companion `.plan.yaml`
