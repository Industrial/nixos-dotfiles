...
{pkgs, ...}: let
  ...
  # data dir. Whisparr rewrites config.xml on UI changes, so the repo file is
  # made group-writable for the service's 'data' group — owner-side git
  # access is preserved.
  dotfilesRepo = "/data/dotfiles";
  configSource = "${dotfilesRepo}/features/media/whisparr/config/config.xml";
in {
  environment = {
  ...
  system.activationScripts.whisparrConfig = {
    text = ''
      source="${configSource}"
      target="${directoryPath}/config.xml"
      if [ ! -f "$source" ]; then
        echo "whisparr: $source missing (is ${dotfilesRepo} cloned?); keeping existing config" >&2
      else
