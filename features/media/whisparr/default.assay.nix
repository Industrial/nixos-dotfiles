# Colocated suite: whisparr service + declarative config symlink wiring.
let
...
in
  assay.suite "whisparr" {
    ...
    activationScriptDeclared =
      assay.eq (mod.system.activationScripts ? whisparrConfig) true;
    linksDeclarativeConfig = assay.eq
      (builtins.match ".*/data/dotfiles/features/media/whisparr/config/config.xml.*" activation != null)
      true;
    ...
    repoConfigHasPort = assay.eq
      (builtins.match ".*<Port>6969</Port>.*" configSrc != null)
      true;
