...
pkgs.writeShellApplication {
  ...
  text = ''
    ...
    if [[ -z "$root" || ! -f "$root/rust/Cargo.toml" ]]; then
      echo "nixfetch: set DOTFILES_ROOT to your .dotfiles checkout (need rust/Cargo.toml)" >&2
      exit 127
    fi
    manifest="$root/rust/Cargo.toml"
    bin="$root/rust/target/release/nixfetch"
    debug="$root/rust/target/debug/nixfetch"
    if [[ ! -x "$bin" ]]; then
      bin="$debug"
    fi
    needs_build=0
    ...
    if [[ "$needs_build" -eq 1 ]]; then
      echo "nixfetch: building (cargo -p nixfetch --release)…" >&2
      env -u RUSTC_WRAPPER cargo build --release --manifest-path "$manifest" -p nixfetch
      bin="$root/rust/target/release/nixfetch"
    fi
    exec "$bin" "$@"
  '';
