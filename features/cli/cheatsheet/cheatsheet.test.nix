{pkgs, ...}: let
  ...
  cheatsheetModule = import ./default.nix {pkgs = mockPkgs;};
in {
  # Test that the module evaluates without errors
  testModuleEvaluates = {
    expr = builtins.length cheatsheetModule.environment.systemPackages;
    expected = 1;
  };

  # Test that the package is created with correct attributes
  testPackageAttributes = {
    expr = (builtins.head cheatsheetModule.environment.systemPackages).name;
    expected = "cheatsheet";
  };

  # Test that the package has the correct version
  testPackageVersion = {
    expr = (builtins.head cheatsheetModule.environment.systemPackages).version;
    expected = "1.0";
  };
